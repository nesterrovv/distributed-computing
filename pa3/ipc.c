...
int send_multicast(void* self, const Message* sending_message) {
    ...
    for (local_id destination_id = 0; destination_id < MAXIMAL_PROCESSES_NUMBER; destination_id = destination_id + 1) {
...
int receive_any(void* source, Message* receiving_message) {
    for (;;) {
        for (int current_receiver = 0; current_receiver < MAXIMAL_PROCESSES_NUMBER; current_receiver = current_receiver + 1) {
